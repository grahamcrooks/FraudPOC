# Reference data

Practice and practitioner records used as test data for the claim form's Practice and Practitioner dropdowns (see [claim data model](claim-data-model.md)). The first two practices match the test providers in [configure provider data object](../prompts/configure-provider-data-object.md).

Values marked "(not shown)" were not available in the source material.

## Practices

| Practice Name | Provider ID | ABN | Type | Address | Phone | City |
| --- | --- | --- | --- | --- | --- | --- |
| Brighton Dental Care | 1948302K | 74 291 836 540 | Dental | 12 Bay St, Brighton VIC 3186 | (03) 9592 3344 | Brighton |
| Valley Physio & Rehab Centre | 7291048H | 91 632 847 501 | Physio | 47 Railway Parade, Ferntree Gully VIC 3156 | (03) 9758 4429 | Ferntree Gully |

## Practitioners

| Practitioner Name | Credentials | AHPRA/Registration | Associated Practice | Type |
| --- | --- | --- | --- | --- |
| Dr. Wendy Kostadinov | BDS | (not shown) | Brighton Dental Care | Dentist |
| Amy Tran | B.Physio, APA | PHY0004827193 | Valley Physio & Rehab Centre | Physiotherapist |

## Excluded records

Four practices and four practitioners from the original README table were deliberately left out because they appear to be real businesses and people: ABC Dentistry, QML Pathology, Best Remedial Services Chermside and Dr Rachel Nowland Medical, with their associated practitioners. Replace them with fictional records if the demo needs more than two practices. The originals remain in the git history before this restructure.
